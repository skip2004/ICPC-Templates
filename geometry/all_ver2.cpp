...
db area(const std::vector<vec2> & x) {
	db res = 0;
...
circle incircle(vec2 a, vec2 b, vec2 c) { // 三点确定内心
	db A = (b - c).abs(), B = (c - a).abs(), C = (a - b).abs();
	return {(a * A + b * B + c * C) / (A + B + C), fabs((b - a) * (c - a)) / (A + B + C)};
}
circle circumcenter(vec2 a, vec2 b, vec2 c) { // 三点确定外心
