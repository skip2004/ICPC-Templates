...
\codesectiondesc{\subsection}{cpp}{contents/math/power-projection.cpp}{Power Projection}{ $deg(f) = deg(g) = n - 1, g(0) = 0$，返回 $ans[i] = [x^{n-1}] f(x) g(x)^{i}, i = 0, \cdots, n-1$}
...
\codesectiondesc{\subsection}{cpp}{contents/string/kmp.cpp}{KMP}{$s.size()>0$, $b_i$ 为最大的 $j<i$，使得 $s[:j] = s[i-j:i]$}
...
\codesectiondesc{\subsection}{cpp}{contents/misc/montgomery.cpp}{Montgomery}{要求 $p$ 是奇数，不要超过 $2^{62}$。reduce 计算 $\frac{x}{2^{64}} \pmod p$，如果输入在 $[0, p \times 2^{64})$ 内，返回值在 $[0, 2p)$ 内。}
\codesectiondesc{\subsection}{cpp}{contents/misc/pragma.cpp}{Pragma}{}
