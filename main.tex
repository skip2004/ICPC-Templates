...
\subsection{二分图匹配}
\subsubsection{\href{run:./contents/graph/matching/dfs-matching.cpp}{二分图最大匹配 | 最小边覆盖}}
左到右连单向边，时间复杂度 $O(M|match|)$。

\inputminted{cpp}{contents/graph/matching/dfs-matching.cpp}

\subsubsection{\href{run:./contents/graph/matching/bitset-dfs-matching.cpp}{二分图最大匹配 (bitset 优化)}}
左到右连单向边，时间复杂度 $O(n^2/w\ |match|)$。

\inputminted{cpp}{contents/graph/matching/bitset-dfs-matching.cpp}

\subsubsection{\href{run:./contents/graph/matching/HK.cpp}{二分图最大匹配 (Hopcroft-Karp) \& 最小边覆盖}}
左到右单向边，时间复杂度 $O(M \sqrt{|match|})$

\inputminted{cpp}{contents/graph/matching/HK.cpp}

\subsubsection{\href{run:./contents/graph/matching/KM.cpp}{二分图最小权匹配}}

返回左边每个点匹配的右边点的编号，左边的点不多于右边的点（必要时补点）。

\inputminted{cpp}{contents/graph/matching/KM.cpp}

最小权匹配。返回代价和、右边每个点匹配的左边点的编号、对应的边权。过程中能计算出匹配为 $0 \cdots |match|$ 的最优解。

时间复杂度 $O(|match|(L^2+|E|))$。
\inputminted{cpp}{contents/graph/matching/newkm.cpp}

\subsection{\href{run:./contents/graph/dinic.cpp}{网络最大流 (dinic)}}
\inputminted{cpp}{contents/graph/dinic.cpp}

\subsection{\href{run:./contents/graph/MCMF.cpp}{最小费用流}}
\inputminted{cpp}{contents/graph/MCMF.cpp}

\subsection{\href{run:./contents/graph/带花树.cpp}{一般图最大匹配 (带花树)}}
edge 里存双向边。运行结束后 match 存放每个点匹配的点的编号，如果没有匹配则为 $0$。
\inputminted{cpp}{contents/graph/带花树.cpp}

\subsection{\href{run:./contents/graph/D-MST.cpp}{最小树形图}}

用 DMST::link 添加边，然后调用 DMST::solve(root) 得到最小树形图边的集合，如果没有合法解会返回空集 (注意 $n=1$)。

\inputminted{cpp}{contents/graph/D-MST.cpp}

\subsection{\href{run:./contents/graph/kosaraju.cpp}{强连通分量 (kosaraju)}}

e 存有向边，re 里存反向边。用 solve 得到所有强连通分量。时间复杂度 $O(\frac{n^2}{w})$。

\inputminted{cpp}{contents/graph/kosaraju.cpp}

\subsection{\href{run:./contents/graph/eulerwalk.cpp}{欧拉回路/欧拉路径}}
...
\subsection{\href{run:./contents/graph/支配树.cpp}{支配树}}
...
\subsection{\href{run:./contents/numbertheory/approx.cpp}{取模还原分数}}
\inputminted{cpp}{contents/numbertheory/approx.cpp}

\subsection{\href{run:./contents/numbertheory/exgcd.cpp}{扩展欧几里得}}
结果的 $x, y$ 满足：$ax + by = \gcd(a, b)$; 若 $a, b$ 非 $0$，则： $-b \leq x \leq b, -a \leq y \leq a$。

\inputminted{cpp}{contents/numbertheory/exgcd.cpp}

\subsection{\href{run:./contents/numbertheory/Euclid.cpp}{万能欧几里得}}
\inputminted{cpp}{contents/numbertheory/Euclid.cpp}

\subsection{\href{run:./contents/numbertheory/floor_sum.cpp}{floor-sum}}
$n < 2 ^ {32}, 1 \leq m < 2 ^ {32}$
...
\subsection{\href{run:./contents/numbertheory/minmod.cpp}{Min of Mod of Linear}}
$$
...
\subsection{\href{run:./contents/numbertheory/sbt.cpp}{Stern-Brocot Tree 二分}}
\inputminted{cpp}{contents/numbertheory/sbt.cpp}

\subsection{\href{run:./contents/numbertheory/CRT.cpp}{中国剩余定理}}
返回满足 $x \equiv a_i \pmod{m_i} (i = 1, 2)$ 的最小非负整数 $x$，若无解则返回 $-1$。

\inputminted{cpp}{contents/numbertheory/CRT.cpp}

\subsection{\href{run:./contents/numbertheory/Miller_Rabin.cpp}{Miller-Rabin}}

调用 checkprime 判断是否为质数。

\inputminted{cpp}{contents/numbertheory/Miller_Rabin.cpp}

\subsection{\href{run:./contents/numbertheory/pollard_rho.cpp}{Pollard-rho}}

调用 factor 获得 $x$ 质因子的可重集合（已从小到大排序）。

\inputminted{cpp}{contents/numbertheory/pollard_rho.cpp}

\subsection{\href{run:./contents/numbertheory/quadres.cpp}{二次剩余}}
quadres 用于判断是否存在二次剩余，sqrtp 用于计算结果。$p$ 或 $b$ 需要是质数（或者 $1$）。

高斯引理： 
$$
\left( \frac{a}{p} \right) = (-1)^{\sum_{i=1}^{\frac{p-1}{2}} \mathbf{1}[(ai \bmod p) > p/2]}
$$
...
\inputminted{cpp}{contents/numbertheory/quadres.cpp}
...
\subsection{\href{run:./contents/math/FFT.cpp}{Fast Fourier Transform}}
\inputminted{cpp}{contents/math/FFT.cpp}

\subsection{\href{run:./contents/math/NTT.cpp}{Number Theoretic Transform}}
\inputminted{cpp}{contents/math/NTT.cpp}

\subsection{\href{run:./contents/math/GF_noExp.cpp}{Generating function}}
\inputminted{cpp}{contents/math/GF_noExp.cpp}

\subsection{\href{run:./contents/math/online_convolution.cpp}{全在线卷积}}
\inputminted{cpp}{contents/math/online_convolution.cpp}

\subsection{常系数线性递推}

\subsubsection{\href{run:./contents/math/BM.cpp}{Berlekamp Massey}}
返回最短的 $c$ 使得： $\forall n \geq size(c) : a_n = \sum_{i=0}^{size(c)-1} c_i a_{n-1-i}$
\inputminted{cpp}{contents/math/BM.cpp}

\subsubsection{\href{run:./contents/math/线性递推形式转换.cpp}{递推式转分式}}
$$
...
\subsubsection{\href{run:./contents/math/bostan_mori.cpp}{Bostan–Mori}}
$$ \left\lbrack x^k\right\rbrack \frac{p(x)}{q(x)} $$

\inputminted{cpp}{contents/math/bostan_mori.cpp}

\subsubsection{\href{run:./contents/math/Fiduccia.cpp}{远处区间值}}
...
\subsection{\href{run:./contents/math/LP.cpp}{线性规划 (单纯形法)}}
...
\subsection{\href{run:./contents/math/Goldensectionsearch.cpp}{黄金三分}}
返回 $f$ 在 $[min(a, c), max(a, c)]$ 上的极大值点 (f 需要是单峰函数)。
\inputminted{cpp}{contents/math/Goldensectionsearch.cpp}

\section{字符串}

\subsection{\href{run:./contents/string/SAM.cpp}{后缀自动机 SAM}}
需要两倍点数量。
\inputminted{cpp}{contents/string/SAM.cpp}

\subsection{\href{run:./contents/string/PAM.cpp}{回文自动机 PAM}}
\inputminted{cpp}{contents/string/PAM.cpp}

\subsection{\href{run:./contents/string/AC自动机.cpp}{AC 自动机}}
\inputminted{cpp}{contents/string/AC自动机.cpp}

\subsection{\href{run:./contents/string/SA.cpp}{SA}}
后缀数组， $sa$ 是个 $0 \sim n-1$ 的排列，$h[i] = lcp(s[sa[i]:], s[sa[i+1]:])$。
\inputminted{cpp}{contents/string/SA.cpp}

\subsection{\href{run:./contents/string/Zalgo.cpp}{Z algorithm}}
返回的 $lcp[i] = lcp(s, s[i:])$。
\inputminted{cpp}{contents/string/Zalgo.cpp}

\subsection{\href{run:./contents/string/manacher.cpp}{Manacher}}
返回每个回文中心的回文半径大小，奇偶分开考虑。
\inputminted{cpp}{contents/string/manacher.cpp}

\subsection{\href{run:./contents/string/minrep.cpp}{最小表示法}}
返回 $s$ 的最小表示的起始位置。可以用 rotate 算出最小表示的字符串。
\inputminted{cpp}{contents/string/minrep.cpp}

\subsection{\href{run:./contents/string/lyndon.cpp}{Lyndon 分解}}
\inputminted{cpp}{contents/string/lyndon.cpp}


\section{数据结构}
\subsection{\href{run:./contents/ds/ex-bit.cpp}{区间加区间求和 树状数组}}
add 区间加，query 区间求和。区间都是闭区间。
\inputminted{cpp}{contents/ds/ex-bit.cpp}

\subsection{\href{run:./contents/ds/zkw-sgt.cpp}{zkw 线段树}}
单点修改区间半群。如果半群没有单位元可以用 qry2。
\inputminted{cpp}{contents/ds/zkw-sgt.cpp}

\subsection{\href{run:./contents/ds/rangequery.cpp}{静态区间半群}}
$O(n \log n)$ 预处理，$O(1)$ 查询。
\inputminted{cpp}{contents/ds/rangequery.cpp}

\subsection{\href{run:./contents/ds/LCT.cpp}{Link Cut Tree}}
\inputminted{cpp}{contents/ds/LCT.cpp}

\subsection{\href{run:./contents/ds/btrie.cpp}{压位 Trie}}
\inputminted{cpp}{contents/ds/btrie.cpp}

% \subsection{FHQ Treap} % !!! pb
% \inputminted{cpp}{contents/ds/fhq.cpp}

\subsection{\href{run:./contents/ds/pbds_tree.cpp}{pbds tree}}
\inputminted{cpp}{contents/ds/pbds_tree.cpp}


\section{geometry}
\subsection{\href{run:./contents/geometry/向量.cpp}{向量}}
\inputminted{cpp}{contents/geometry/向量.cpp}

\subsection{\href{run:./contents/geometry/centers.cpp}{三角形各心}}
\inputminted{cpp}{contents/geometry/centers.cpp}

\subsection{\href{run:./contents/geometry/直线半平面.cpp}{直线半平面}}
\inputminted{cpp}{contents/geometry/直线半平面.cpp}

\subsection{\href{run:./contents/geometry/半平面交.cpp}{半平面交}}
\inputminted{cpp}{contents/geometry/半平面交.cpp}

\subsection{\href{run:./contents/geometry/线段.cpp}{线段}}
\inputminted{cpp}{contents/geometry/线段.cpp}

\subsection{\href{run:./contents/geometry/简单多边形.cpp}{简单多边形}}
\inputminted{cpp}{contents/geometry/简单多边形.cpp}

\subsection{\href{run:./contents/geometry/简单多边形三角剖分.cpp}{简单多边形三角剖分}}
\inputminted{cpp}{contents/geometry/简单多边形三角剖分.cpp}

\subsection{\href{run:./contents/geometry/seg_in_polygon.cpp}{线段 in 多边形}}
\inputminted{cpp}{contents/geometry/seg_in_polygon.cpp}

\subsection{\href{run:./contents/geometry/图形交.cpp}{图形求交}}
\inputminted{cpp}{contents/geometry/图形交.cpp}

\subsection{\href{run:./contents/geometry/凸包.cpp}{凸包}}
结果为逆时针。
\inputminted{cpp}{contents/geometry/凸包.cpp}

\subsection{\href{run:./contents/geometry/上凸壳.cpp}{上凸壳}}
结果显然为顺时针。
\inputminted{cpp}{contents/geometry/上凸壳.cpp}

\subsection{\href{run:./contents/geometry/最小圆覆盖.cpp}{最小圆覆盖}}
\inputminted{cpp}{contents/geometry/最小圆覆盖.cpp}

\subsection{\href{run:./contents/geometry/最近点对.cpp}{最近点对}}
\inputminted{cpp}{contents/geometry/最近点对.cpp}

\subsection{\href{run:./contents/geometry/凸包直径.cpp}{凸包直径}}
\inputminted{cpp}{contents/geometry/凸包直径.cpp}
% https://codeforces.com/blog/entry/48868?#comment-329264

\subsection{\href{run:./contents/geometry/切凸包.cpp}{切凸包}}
\inputminted{cpp}{contents/geometry/切凸包.cpp}

\subsection{\href{run:./contents/geometry/voronoi.cpp}{V 图}}
\inputminted{cpp}{contents/geometry/voronoi.cpp}

\subsection{\href{run:./contents/geometry/Delaunay.cpp}{Delaunay 三角剖分}}
\inputminted{cpp}{contents/geometry/Delaunay.cpp}

\section{geometry3d}
\subsection{\href{run:./contents/geometry/3d/vector.cpp}{向量}}
\inputminted{cpp}{contents/geometry/3d/vector.cpp}

\subsection{\href{run:./contents/geometry/3d/plane.cpp}{平面}}
\inputminted{cpp}{contents/geometry/3d/plane.cpp}

\subsection{\href{run:./contents/geometry/3d/line.cpp}{直线}}
\inputminted{cpp}{contents/geometry/3d/line.cpp}

\subsection{\href{run:./contents/geometry/3d/convex.cpp}{凸包}}
\inputminted{cpp}{contents/geometry/3d/convex.cpp}


\section{Misc}
\subsection{\href{run:./contents/misc/pragma.cpp}{Pragma}}
\inputminted{cpp}{contents/misc/pragma.cpp}

\subsection{\href{run:./contents/misc/Barrett.cpp}{Barrett}}
\inputminted{cpp}{contents/misc/Barrett.cpp}

\subsection{\href{run:./contents/misc/fastdivcheck.cpp}{快速整除判定}}
要求 $p$ 是奇数。如果是 (u)int128，求逆循环次数要改成 $6$。
\inputminted{cpp}{contents/misc/fastdivcheck.cpp}

\subsection{\href{run:./contents/misc/lcs.cpp}{LCS}}
\inputminted{cpp}{contents/misc/lcs.cpp}

\subsection{\href{run:./contents/misc/zeller.cpp}{日期公式}}
getday 返回自 $1/1/1$ 起到 $y/m/d$ 的天数，$1/1/1$ 是星期一，所以模 $7$ 可以得算出是周几。

date 是 getday 的逆。
\inputminted{cpp}{contents/misc/zeller.cpp}

\subsection{\href{run:./contents/misc/xorshift.cpp}{Xorshift}}
\inputminted{cpp}{contents/misc/xorshift.cpp}

\subsection{distributions}

\subsection{\href{run:./contents/misc/template.py}{python}}
\inputminted{py}{contents/misc/template.py}

\subsection{\href{run:./contents/misc/ieee754.txt}{浮点数精度}}
